...
- Never run the full validation suite; focused, filtered runs only, and only to settle a factual question. Installing the .NET SDK is not part of this workflow — if no SDK is present, trace statically and say so.
