...
`scripts/test.sh` runs all test projects in order; `--no-build` is valid only because the Release build immediately above covered this exact tree; drop it and rebuild if any file changed since. Every test project must pass. If any fails:
