...
name: gh-implement
description: >-
  Orchestrator for implementing a Reihitsu GitHub issue end-to-end in a Claude Code Cloud Agent environment. Triggers when the initial prompt references a GitHub issue (e.g. "implement #123", "fix issue 45", or a github.com/.../issues/N URL) and the work must be carried out from a clean cloud sandbox. It claims the issue by opening a generic-placeholder draft PR, prepares the toolchain through the repository scripts, proves every bug report through a **reproduction gate** subagent that writes and runs the failing regression test before any analysis — a report that does not reproduce ends the run right there with that test, a `Refs`-linked PR and a question to the user, instead of paying for a contract, an implementation and an audit — triages the run, runs the `gh-rubber-duck` Behavior Contract gate in a dedicated read-only subagent before touching any further file on every behavioral run, requires a code-derived defect-class enumeration and candidate sweep for bug reports plus a guard-delta and predicate-boundary analysis for every moved predicate, freezes a scope ledger at contract acceptance, turns the accepted contract into the regression matrix, delegates the change to the matching repository slash command, updates the draft after focused commits, records a falsifiable admission artifact before any audit, synchronizes `origin/main`, spends at most two official `gh-preflight` attempts when the trigger list requires one, runs the full validation suite once, fully rewrites the PR title and description, and pushes the single CI trigger. GitHub operations use the GitHub MCP server; the `gh` CLI is not installed. Do NOT use locally when the SDK is already installed and the user is driving the workflow interactively.
---
...
3. Claim the issue through the existing ownership workflow when it is unclaimed, then prepare the toolchain.
4. Gather the neutral **evidence bundle** once; every isolated agent in this run consumes that same bundle.
5. On a **bug report**, run the **reproduction gate** in one `reihitsu-reproduction` subagent before any other analysis — unless an existing characterization test already settles it, which one focused run proves. `NO SCENARIO` and `BLOCKED` end the run here, and so does a `NOT REPRODUCED` once the escalated confirmation agrees.
6. Triage the run as **routine** or **behavioral**, and record which gates that decision keeps.
7. Read `.claude/skills/gh-rubber-duck/SKILL.md` completely in this parent agent (behavioral runs).
...
Claiming the issue (steps 1–3) happens before the reproduction gate, the triage, and the contract gate so ownership is never lost while analysis runs. The toolchain preparation fits between the claim and the first `dotnet` command — the reproduction gate is the first thing that needs it.
...
The cloud sandbox is **Linux** and does **not** ship with the .NET SDK. Claim the issue first; then, before doing anything that touches `dotnet`, prepare the toolchain through the repository script:

```bash
scripts/prepare.sh
```

It probes `dotnet --list-sdks` and installs .NET 10 into `$HOME/.dotnet` only when no `10.*` SDK is present, so it is a no-op on an image that already ships one. The repository targets `net10.0` and there is no `global.json`; never fall back to an older SDK.

The other repository scripts — `scripts/build.sh`, `scripts/test.sh`, `scripts/format.sh`, `scripts/verify-text-only.sh` — resolve the SDK the same way, so they work whether or not `PATH` carried over from an earlier step. Use them instead of hand-written `dotnet` invocations; they are the single owner of these commands across `CLAUDE.md`, `AGENTS.md`, and every workflow skill.

If the SDK cannot be installed (no network egress, mirror missing), record the failure in the already-open draft PR's `Review notes` and stop. Do not proceed with a partial validation — a green run without the SDK is meaningless.
...
## Subagent tiers live in `.claude/agents/`

Every gate below is spawned by **`subagent_type`**, never by an ad-hoc prompt with a model argument attached:
...
`.claude/agents/`, which is their single owner — see `.claude/agents/README.md` for the current assignment and
the reasoning behind it. Do not pass a `model` argument when spawning; the one exception is the escalated
reproduction confirmation, which deliberately overrides the tier upward and is described where it applies.

Do not re-tune a stage by editing this file. Change the agent's frontmatter instead, so the assignment stays
inspectable in one place rather than inferred from three workflow prompts.

You are the orchestrator and you run in the session agent, not in a definition here. Nothing in this section
applies to you: you own the `Never` rules, the scope ledger, and the audit budget, and those do not move to a
cheaper tier.
...
Carry the facts that are awkward or unreliable for an agent to reconstruct:

- the base and head SHAs, the merge base, and the current **remote** `main` SHA with the time you read it;
- the changed-file list and diff, when there is one;
- your proof that the local checkout matches that head;
- the result of the pre-gate build (see below);
- the reproduction gate's observed facts, on a bug report;
- the previous reviewer report, when this is a preflight retry;
- the user's chat clarifications, quoted verbatim — nothing else in the run can reach them.

**Reference the documents; do not inline them.** The issue and the PR go into the bundle as
`thoenissen/Reihitsu#<N>` with the instruction to read them through `mcp__github__issue_read` and
`mcp__github__pull_request_read`. Isolation requires that an agent not see your *reasoning*; it has never
required pasting a document that one MCP call retrieves. Pasting the issue body into four prompts pays for it
four times and, worse, silently freezes a document that may have gained a clarifying comment since.
...
### Build once, before any expensive agent

`scripts/build.sh` takes roughly eleven seconds on this repository, and this repository dogfoods its own
analyzers with `TreatWarningsAsErrors=true` — so "could this combination break the build?" is a question an
audit can otherwise spend a great deal of reasoning on. Run it, and put its result in the bundle as a fact:

```bash
scripts/build.sh
```

Do this before the first reviewer spawn and again on any tree you hand to an audit. A red build is never
something to hand to a gate — fix it first. The build result does not replace the full validation, which runs
once, later, on the final tree.
...
### The characterization carve-out

This repository regularly ships a **characterization test** — one that asserts the broken output on purpose,
so the defect cannot widen silently — and then files the issue that asks for the fix. When such a test
exists, the reproduction is an established fact in the repository before the run starts, and spending a
subagent to re-establish it buys nothing.

The carve-out is decidable, and it is settled by **running** the test rather than by reading it:

1. `rg` for the issue's rule ID, phase, helper, or construct in the test projects and name the candidate test.
2. Read it and confirm it asserts the issue's reported **observed** output for the issue's reported input — not
   merely that it touches the same rule. A test that asserts the *expected* output is not a characterization
   of this defect; it is a passing test of something else.
3. Run it:

   ```bash
   scripts/test.sh --project <project> --filter "FullyQualifiedName~<test>"
   ```

   It must be **green on the current head**. Green proves the repository still produces the reported output —
   which is exactly what the gate would have proven, in seconds instead of a subagent.

All three hold → record `REPRODUCED (characterized)` with the test's path, name, helper, command, and the
passing run's summary line as the gate's observed facts, seed the regression matrix with that test, and
continue with the triage. The contract still decides the real coverage, and rewriting the characterization
test's assertion from the broken output to the fixed one is the implementation's first step.

Any one of them fails — no such test, it asserts something else, or it is **red** — and the full gate runs as
written. A red candidate is the important case: it means the repository no longer does what the test recorded,
so the defect has changed and the stale test is evidence of nothing. Never carve out on a test you did not run
on this head.

Steps 1–3 cost one focused test run. Do not skip step 3 to save it.

### Why it runs before the contract

[`fix-formatter`](../../commands/fix-formatter.md) and [`fix-analyzer-rule`](../../commands/fix-analyzer-rule.md) already require a failing regression test before production code, and `CLAUDE.md` requires it repository-wide. The gate therefore adds no work to the reproducing path — it moves work that has to happen anyway to the front and into a subagent, and it turns the reproduction into an *input* to the Behavior Contract instead of something the contract has to assume from prose. On the non-reproducing path it removes the entire remainder of the workflow.

### The subagent

Spawn **one** fresh subagent of type `reihitsu-reproduction`. Its model and effort come from
`.claude/agents/reihitsu-reproduction.md` — pass no model argument, so the tier stays tunable in one place.
It is the only writer of the reproduction test; the parent stays the only writer of commits, pushes, and GitHub state.
...
**`NOT REPRODUCED`** — the literal example is green, and so is every variant of the fixed fan-out below, **and the escalated confirmation agrees**.
...
Green on all of them is not yet the verdict. The gate's three other outcomes are cheap to reverse — `REPRODUCED` is falsified by the contract, the implementation, and the audit that all run against that test, and `BLOCKED` costs one restart — but `NOT REPRODUCED` ends the run, downgrades the issue link, and hands the user a question, so a wrong one is expensive and quiet. **Escalate it once before acting on it:**

1. Re-run the **same** `reihitsu-reproduction` agent type, as a fresh subagent, with a per-invocation `model` override of `opus`.
2. Hand it the same evidence bundle plus the first run's complete report — its test file, name, helper, commands, scenarios, and raw results — and ask it to audit that run against the repository and return its own verdict, not to agree with it.
3. Exactly one escalation. The escalated verdict decides, and it costs a **process start**, never a second gate result.

If the environment offers no per-invocation model override, perform that confirmation yourself rather than acting on an unconfirmed non-reproduction.

An escalation that returns `REPRODUCED`, `NO SCENARIO`, or `BLOCKED` replaces the verdict and the run continues down that path instead. A confirmed `NOT REPRODUCED` ends the run:
...
7. Ask how to proceed (`AskUserQuestion`), with these choices: request that specific evidence from the issue author; accept the current behavior as correct and close the issue as not-reproducible; or continue into the full contract gate anyway because the user has reason to believe the harness, not the report, is wrong.
...
| Test name | `…` |
| Helper | `…` |
| Command | `scripts/test.sh --project … --filter …` |
...
From that moment the run is behavioral: the earlier routine classification is void, the contract gate applies in full, and the preflight decision is taken again from the trigger list against the new diff.
...
Run the analysis in **one fresh subagent** of type `reihitsu-rubber-duck`, with no access to your reasoning. Isolation is the point: an analysis primed with your intended fix will confirm it instead of challenging it. Its model and effort come from `.claude/agents/reihitsu-rubber-duck.md`; pass no model argument.
...
On a routine run the artifact keeps the rows that apply to text — referenced paths and links resolve, YAML frontmatter parses, `git diff --check` is clean, nothing contradicts `AGENTS.md`/`CLAUDE.md` or a neighboring skill — and marks the rest N/A.
...
5. Run `scripts/format.sh` over every conflict-resolved and changed C# path.
6. Run the focused tests affected by the merge.
7. Commit and push the synchronized head with `[skip ci]`.
8. Run `scripts/build.sh` on that head and record the result in the evidence bundle.
9. Take the preflight decision against that exact head.

### Re-verify the remote immediately before each reviewer spawn

A remote-tracking ref proves only when you last fetched. An audit spawned against a stale one runs for
minutes and comes back `BLOCKED — state mismatch`, having produced no gate result at all — the single most
expensive way this workflow can waste an agent. Query the **remote** instead, in the same step that spawns the
agent:
...
It costs about a second. If the SHA it prints is not the `origin/main` you merged, do the merge again before
spawning; if it matches, put it in the bundle together with the time you read it. This is not a substitute for
the synchronization above — it is the check that the synchronization is still true at spawn time, and it
applies to every reviewer spawn in the run, including the retry.
...
Run this in a subagent of type `reihitsu-validate` when subagents are available, passing the tree it must
validate and whether anything changed since the Release build. It runs the two scripts and returns pass/fail
per step plus the failing assertions verbatim, which keeps several thousand lines of build and test output
out of this context; its model comes from `.claude/agents/reihitsu-validate.md`, so pass no model argument.
It fixes nothing and diagnoses nothing — you own every repair. Without subagents, run the two commands here
and capture the output to a file instead of into the transcript.

`scripts/test.sh` runs all four test projects in order; `--no-build` is valid only because the Release build immediately above covered this exact tree; drop it and rebuild if any file changed since. All four test projects must pass. If any fails:
...
4. Report back in chat, stating at least:
   ...
   - the reproduction gate outcome (`REPRODUCED`, `REPRODUCED (characterized)`, `NOT REPRODUCED`, `NO SCENARIO`, `BLOCKED`, or N/A for a non-bug issue), the scenarios it ran with the helper each used, and — on the carve-out — the existing test's name and the passing run that established it; on `NOT REPRODUCED` — the `Refs #<N>` downgrade, every gate skipped with that as the reason, and the evidence that would change the verdict;
   ...
   - the run metrics, so the workflow itself can be evaluated after a dozen runs, keeping these apart rather than reporting one number: **official gate verdict attempts**, **reviewer process starts**, state mismatches, tool or environment failures, and elapsed time — plus token cost per gate where the environment reports it, **and the model tier and effort level each gate actually ran at**, without which a later tier change cannot be shown not to have regressed a verdict — plus the reproduction gate outcome, whether its verdict was escalated and what the escalation returned, contract gate result, number of contract rows, defect-class candidates and reproductions, delta-table rows, tests added, findings on the first official preflight, and whether the retry was needed;
...
- Take the characterization carve-out when it applies: an existing green test that asserts the reported output settles the same question for one focused run instead of a subagent.
- Check the remote before every reviewer spawn. One second of `ls-remote` is the cheapest thing in the workflow, and it is what stops a complete audit from returning `BLOCKED — state mismatch` with no gate result at all.
...
- Derive the checklist applicability once and hand it over, instead of letting each reviewer rediscover the same six negatives.
...
There is deliberately **no token budget that stops a run**. The expensive stages are already bounded by
counted budgets — one contract subagent, one preflight attempt plus one retry, one reproduction gate plus one
escalation — and those bounds are mechanically decidable, whereas "this run has become too expensive" is not
something a run can decide about itself from the inside. Report the per-gate cost instead (below) and let the
question be answered from a dozen runs. What *does* stop a run mid-flight is a scope checkpoint, and that
already exists.
...
- **Never** take the characterization carve-out from reading a test. Run it on this head and see it green; a red candidate means the defect changed and the gate runs in full.
- **Never** spawn a reviewer without `git ls-remote origin refs/heads/main` in that same step, and never spawn one on a red build.
- **Never** paste the issue or PR body into a subagent prompt. Reference it by number — isolation is about your reasoning, not about documents an MCP call retrieves.
...
- **Never** act on a `NOT REPRODUCED` — end the run, downgrade the link, ask the user — before the single escalated confirmation has agreed with it. The other three outcomes are reversible by the stages that follow; this one is not.
- **Never** spawn a gate with an ad-hoc prompt or an inline model argument instead of its `subagent_type` from `.claude/agents/`. The escalated reproduction confirmation is the one deliberate override, and it only ever moves the tier upward.
- **Never** let the validation subagent edit, commit, or diagnose. It runs two scripts and reports; every repair is yours.
...
- **Never** claim a comment-only carve-out from reading the diff. Run `scripts/verify-text-only.sh` and quote its proof line, or treat the diff as behavioral.
...
- **Never** modify `global.json` or the `TargetFramework` to dodge an SDK install — run `scripts/prepare.sh` instead.
...
- [ ] Toolchain prepared via `scripts/prepare.sh`
...
- [ ] Evidence bundle gathered once, referencing the issue and PR by number rather than inlining them, carrying no proposed solution or suspected cause
- [ ] `scripts/build.sh` green and recorded in the bundle before the first reviewer spawn
- [ ] `git ls-remote origin refs/heads/main` re-checked in the step that spawns each reviewer, and its SHA recorded with the time
- [ ] Checklist-applicability list derived from the diff's shape and handed to the audit
- [ ] Bug report: reproduction gate run in one `reihitsu-reproduction` subagent before any other analysis, its outcome recorded, and the seed test committed with `[skip ci]` — or `REPRODUCED (characterized)` recorded against an existing test that was **run green on this head** — non-bug issue: gate recorded N/A
...
- [ ] Behavioral run: `gh-rubber-duck/SKILL.md` read in this agent and exactly one read-only `reihitsu-rubber-duck` subagent spawned with that bundle before any further edit — routine run: contract note written before any edit
...
- [ ] Change made, files formatted via `scripts/format.sh`, focused tests green
...
- [ ] Final chat report states the reproduction gate outcome, the scope classification with its reason, the contract gate result, official attempts and reviewer process starts separately, scope growth across repair cycles, and whether the budget was exhausted
