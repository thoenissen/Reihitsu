...
6. Run the read-only `gh-preflight` skill against the current pushed PR head. Fix every confirmed finding and the complete defect class in `[skip ci]` commits, then rerun preflight until it returns `PASS`. Do not post preflight findings through GitHub MCP.
7. Run the full validation suite (build + all four test projects). Do not skip projects. Commit any fixes with `[skip ci]` too.
8. Push any validation fix commits, then push one final commit *without* `[skip ci]` (an empty `Ready for CI (#$ARGUMENTS)` commit works) so CI runs exactly once, now that everything is finished. Then fully rewrite both the PR **title** and **body** from the actual implementation — never leave the claim-time placeholder title or wording in place. Keep the PR draft for the human reviewer.
