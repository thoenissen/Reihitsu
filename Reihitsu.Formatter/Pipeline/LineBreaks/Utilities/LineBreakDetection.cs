...
internal static class LineBreakDetection
{
    ...
    /// <remarks>
    /// This is the single owner of the normalize-or-leave decision for accessor lists with bodies or
    /// interior trivia, shared by every rewriter that places accessor-list braces. Auto-accessor lists
    /// such as <c>{ get; set; }</c> are not normalized here, unless the list itself carries a comment or a
    /// directive: <see cref="AccessorListLayout"/> decides whether a property or indexer auto-accessor list
    /// collapses, stays as written, or is laid out, and an event auto-accessor list keeps its existing
    /// layout. The interior-scoped check is deliberate: a comment trailing the closing brace sits outside
    /// the list and must not force the braces apart. The collapse path applies a second guard of its own,
    /// which is interior-scoped for the same reason, so a comment following the accessor list is not a
    /// join barrier.
    /// </remarks>
