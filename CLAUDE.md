...
The repository scripts under `scripts/` are the canonical entry points. Claude runs on Linux, so use the `.sh` variants. Each one resolves the .NET SDK itself — it probes `dotnet --list-sdks` and installs .NET 10 into `$HOME/.dotnet` only when no `10.*` SDK is present — so they work unchanged in a fresh cloud sandbox and on a developer machine. See `scripts/README.md`.

```shell
scripts/prepare.sh        # verify the toolchain, installing only if needed
scripts/build.sh          # dotnet build Reihitsu.sln -c Release
scripts/test.sh           # all test projects
```

Focused runs:

```shell
scripts/test.sh --project analyzer --filter "FullyQualifiedName~Reihitsu.Analyzer.Test.Formatting.RH3202ExpressionStyleMethodsShouldNotBeUsedAnalyzerTests.VerifyExpressionBodiedMethodsAreDetectedAndFixed"
scripts/test.sh --project formatter --filter "FullyQualifiedName~Reihitsu.Formatter.Test.Unit.Indentation.LayoutComputerTests.ComputeReturnsNonEmptyModelForSimpleClass"
scripts/test.sh --project core --filter "FullyQualifiedName~Reihitsu.Core.Test.CasingUtilitiesTests.ToCamelCaseTest"
scripts/test.sh --project cli --filter "FullyQualifiedName~Reihitsu.Cli.Test.Unit.ProgramTests.ParseArgumentsUnknownOptionReturnsUnknownOption"
```

`scripts/verify-text-only.sh` proves mechanically that a change carries no compiled behavior — a Roslyn comparison of the token stream, directive and disabled-text structure, literal contents, and changed trivia categories of every changed C# file. The `gh-*` workflows use its verdict to decide whether an expensive audit is required at all.
...
| `/gh-implement` | Implement a GitHub issue with an early branch and draft-PR claim, a reproduction gate for bug reports, and the Behavior Contract gate for behavioral work |
...
- **Reproduction gate** — `gh-implement`'s first stage on a **bug report**, run in one subagent before any other analysis and before any other file is touched. It writes and runs a regression test for the issue's reported scenario and nothing else. `REPRODUCED` (failing on the issue's own expected-versus-actual difference, not on a compile error) commits the test and seeds the regression matrix, and hands the Rubber Duck a confirmed executable reproduction instead of prose. `NOT REPRODUCED` — green on the literal example *and* on the fixed fan-out of LF/CRLF, the counterpart surface, the code-fix path, and the nearest sibling shape — ends the run: the passing test stays as a characterization test, the PR links `Refs #<N>` rather than `Closes #<N>` so a merge cannot close an unexplained bug, the contract and the preflight are skipped, the validation still runs, and the user decides how to proceed. `NO SCENARIO` and `BLOCKED` stop and ask. Because `NOT REPRODUCED` is the only verdict no later stage can falsify, it is confirmed once by re-running the same agent type at a higher tier before the run ends. The gate adds no work to a reproducing run — the bug-fix commands require that test anyway — and removes the three most expensive stages from a non-reproducing one. It is skipped entirely on the **characterization carve-out**: when a test already in the repository asserts the issue's reported observed output for its reported input and is green when run on the current head, the reproduction is an established fact and that test becomes the seed. All three conditions are checked, and the third by running the test — a red candidate means the defect changed, and the full gate runs.
...
- A `Regex` match timeout is a ReDoS guard against untrusted input, not a general safety margin — it is a wall-clock budget, so a thread merely descheduled on a loaded machine can throw even when the engine has done almost no matching work. Inside an analyzer, that throw surfaces as `AD0001` and silently disables the analyzer for the whole compilation; inside a test, it is a spurious failure under CI contention. Default new `Regex` instances to `Regex.InfiniteMatchTimeout`. Only add a bounded timeout when the pattern matches genuinely untrusted input (not a repository-owned file, not source Roslyn already parsed) *and* is capable of catastrophic backtracking (nested or overlapping quantifiers) — and when you do, record both facts in a comment at the declaration so the bound reads as a decision, not a copied neighbour.
