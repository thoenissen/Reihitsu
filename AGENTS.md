...
The repository scripts under `scripts/` are the canonical entry points. Pass `--no-install` so a missing SDK fails instead of being installed — the required SDK is preinstalled in every supported Codex environment and the scripts must not change it. Every script exists as a `.sh` and a `.ps1` variant; see `scripts/README.md`.

```shell
scripts/prepare.sh --no-install   # verify the preinstalled toolchain
scripts/build.sh --no-install     # dotnet build Reihitsu.sln -c Release
scripts/test.sh --no-install      # all four test projects
```

Focused runs:

```shell
scripts/test.sh --no-install --project analyzer --filter "FullyQualifiedName~Reihitsu.Analyzer.Test.Formatting.RH3202ExpressionStyleMethodsShouldNotBeUsedAnalyzerTests.VerifyExpressionBodiedMethodsAreDetectedAndFixed"
scripts/test.sh --no-install --project formatter --filter "FullyQualifiedName~Reihitsu.Formatter.Test.Unit.Indentation.LayoutComputerTests.ComputeReturnsNonEmptyModelForSimpleClass"
scripts/test.sh --no-install --project core --filter "FullyQualifiedName~Reihitsu.Core.Test.CasingUtilitiesTests.ToCamelCaseTest"
scripts/test.sh --no-install --project cli --filter "FullyQualifiedName~Reihitsu.Cli.Test.Unit.ProgramTests.ParseArgumentsUnknownOptionReturnsUnknownOption"
```
...
- The required .NET SDK is preinstalled. Before builds or tests, run `scripts/prepare.sh --no-install`; it verifies the SDK and changes nothing. Never install an SDK, modify `PATH`, or otherwise change the environment.
