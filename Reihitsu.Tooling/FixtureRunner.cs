...
public static class FixtureRunner
{
    ...
    /// <summary>
    /// Global analyzer-config content that disables both tree-level arms of Roslyn's own generated-code
    /// heuristic: the file-name pattern (<c>*.g.cs</c>, <c>*.designer.cs</c>, …) and the leading
    /// <c>&lt;auto-generated&gt;</c> comment. Every fixture is now named after its real on-disk path (see
    /// <see cref="CreateDocument"/>), and without this override a fixture matching either signal would be
    ...
    /// It does not reach Roslyn's separate, symbol-level <c>[GeneratedCode]</c>-attribute check, which is
    /// independent of the tree and was already unaffected by fixture naming before this override existed
    /// </summary>
    ...
    /// a constant. The project also carries a global analyzer config that disables both tree-level arms of
    /// Roslyn's generated-code heuristic (file name and leading comment — see
    /// <see cref="DisableGeneratedCodeHeuristicConfig"/>), so a fixture that looks generated by either signal is
    /// still analyzed like every other fixture. Every fixture is the project's only document, so a code fix that
    /// inspects sibling documents (RH4001's own collision guard, for example) never observes another fixture.
