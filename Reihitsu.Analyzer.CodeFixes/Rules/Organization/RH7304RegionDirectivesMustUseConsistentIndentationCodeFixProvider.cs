...
/// <summary>
/// Code fix provider for <see cref="RH7304RegionDirectivesMustUseConsistentIndentationAnalyzer"/>
/// </summary>
[Shared]
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(RH7304RegionDirectivesMustUseConsistentIndentationCodeFixProvider))]
public class RH7304RegionDirectivesMustUseConsistentIndentationCodeFixProvider : CodeFixProvider
{
    #region Methods
    ...
    private static async Task<Document> ApplyCodeFixAsync(Document document, SyntaxTrivia directiveTrivia, CancellationToken cancellationToken)
    {
        ...
        var expectedIndentation = RH7304RegionDirectivesMustUseConsistentIndentationAnalyzer.GetExpectedIndentation(syntaxRoot, sourceText, regionTrivia, endRegionTrivia);
    ...
    /// <inheritdoc/>
    public sealed override ImmutableArray<string> FixableDiagnosticIds => [RH7304RegionDirectivesMustUseConsistentIndentationAnalyzer.DiagnosticId];
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
                ...
                || directiveTrivia.RawKind == (int)SyntaxKind.EndRegionDirectiveTrivia)
            {
                context.RegisterCodeFix(CodeAction.Create(CodeFixResources.RH7304Title,
                                                          ...
                                                          nameof(RH7304RegionDirectivesMustUseConsistentIndentationCodeFixProvider)),
                                        diagnostic);
            }
        }
    }
