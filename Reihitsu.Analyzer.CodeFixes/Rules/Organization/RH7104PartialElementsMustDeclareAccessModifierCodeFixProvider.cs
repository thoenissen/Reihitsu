...
/// <summary>
/// Code fix provider for <see cref="RH7104PartialElementsMustDeclareAccessModifierAnalyzer"/>
/// </summary>
[Shared]
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(RH7104PartialElementsMustDeclareAccessModifierCodeFixProvider))]
public class RH7104PartialElementsMustDeclareAccessModifierCodeFixProvider : CodeFixProvider
{
    #region Methods
    ...
    /// <inheritdoc/>
    public sealed override ImmutableArray<string> FixableDiagnosticIds => [RH7104PartialElementsMustDeclareAccessModifierAnalyzer.DiagnosticId];
    ...
    public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        if (root != null)
        {
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                {
                    context.RegisterCodeFix(CodeAction.Create(CodeFixResources.RH7104Title,
                                                              token => ApplyCodeFixAsync(context.Document, typeDeclaration, token),
                                                              nameof(RH7104PartialElementsMustDeclareAccessModifierCodeFixProvider)),
                                            diagnostic);
                }
            }
        }
