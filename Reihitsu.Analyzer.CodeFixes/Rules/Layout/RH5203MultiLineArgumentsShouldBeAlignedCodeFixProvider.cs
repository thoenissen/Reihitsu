...
/// <summary>
/// Providing fixes for <see cref="RH5203MultiLineArgumentsShouldBeAlignedAnalyzer"/>
/// </summary>
[Shared]
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(RH5203MultiLineArgumentsShouldBeAlignedCodeFixProvider))]
public class RH5203MultiLineArgumentsShouldBeAlignedCodeFixProvider : CodeFixProvider
{
    #region Methods
    ...
    /// <inheritdoc/>
    public sealed override ImmutableArray<string> FixableDiagnosticIds => [RH5203MultiLineArgumentsShouldBeAlignedAnalyzer.DiagnosticId];
    ...
    public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        if (root != null)
        {
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                if (argument != null)
                {
                    context.RegisterCodeFix(CodeAction.Create(CodeFixResources.RH5203Title,
                                                              cancellationToken => ApplyCodeFixAsync(context.Document, argument, cancellationToken),
                                                              nameof(RH5203MultiLineArgumentsShouldBeAlignedCodeFixProvider)),
                                            diagnostic);
                }
            }
        }
