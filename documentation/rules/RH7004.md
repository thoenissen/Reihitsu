...
The automatic code fix is not offered when a local function or a labeled statement follows the using declaration. Moving such statements into the new `using` body would hide them from earlier statements and produce non-compiling code, so the conversion has to be done by hand in those cases.
