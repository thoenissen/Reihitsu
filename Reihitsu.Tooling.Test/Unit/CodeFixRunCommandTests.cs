...
public sealed class CodeFixRunCommandTests
{
    ...
    /// <summary>
    /// Verifies that a fixture whose source starts with a leading auto-generated comment is still exercised. This
    /// is the second, name-independent arm of Roslyn's generated-code heuristic — a fixture whose file name looks
    /// ordinary but whose content carries the marker comment must not be silently skipped either
    /// </summary>
    /// <returns>A task representing the asynchronous test operation</returns>
    [TestMethod]
    public async Task ExecuteAsyncProcessesFixtureWithAutoGeneratedHeaderComment()
    {
        // Arrange
        using (var fixture = new TemporaryDirectoryFixture())
        {
            fixture.CreateFile("normal.cs", "// <auto-generated/>\n" + CombinedFieldSource);
            ...
            // Assert
            Assert.AreEqual(ExitCodes.Success, exitCode);
            Assert.Contains("== normal.cs [LF] == fixed", output);
        }
    }
    ...
    public async Task ExecuteAsyncResolvesRenamedDocumentAfterDocumentReplacingCodeFix()
    {
        ...
        using (var fixture = new TemporaryDirectoryFixture())
        {
            ...
            Assert.DoesNotContain("--- a/Mismatch.cs", output);
