    ...
    internal static readonly IReadOnlyDictionary<string, string> Reasons = new Dictionary<string, string>(StringComparer.Ordinal)
                                                                           {
                                                                               ...
                                                                               ["RH5107"] = "The code fix re-indents the continuation line to a column short of (or unrelated to) the aligned column the rule documents; reproduced with several parameter-list shapes, so this is a genuine code-fix defect and not an artifact of this one example.",
                                                                               ["RH5501"] = "The code fix inserts a blank line between the attribute list and the moved declaration; reproduced with several declarations following the attribute, so the fix always adds this line even though the rule's own description only asks for a line break. This is a genuine code-fix defect and not an artifact of this one example.",
                                                                               ["RH5503"] = "The code fix inserts a blank line between the attribute list and the moved declaration; reproduced with several declarations following the attribute, so the fix always adds this line even though the rule's own description only asks for a line break. This is a genuine code-fix defect and not an artifact of this one example.",
                                                                               ["RH7103"] = "The code fix drops a blank line that already separated the two members when it reorders them; reproduced independent of member content, so this is a genuine code-fix defect and not an artifact of this one example."
                                                                           };
