...
internal sealed class FormatCommandHandler
{
    ...
    /// <summary>
    /// Determines whether the specified syntax tree is marked as auto-generated in its header
    /// </summary>
    /// <param name="syntaxTree">The syntax tree to inspect</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests</param>
    /// <returns><see langword="true"/> if a supported auto-generated marker is present in a header comment; otherwise, <see langword="false"/></returns>
    private static bool IsAutoGeneratedSource(SyntaxTree syntaxTree, CancellationToken cancellationToken)
    {
        var root = syntaxTree.GetRoot(cancellationToken);
        var headerTrivia = root.GetFirstToken(includeZeroWidth: true).LeadingTrivia;

        return headerTrivia.Any(trivia => IsCommentTrivia(trivia)
                                          && ContainsAutoGeneratedMarker(trivia.ToString()));
    }

    /// <summary>
    /// Determines whether the specified comment contains a supported auto-generated marker
    /// </summary>
    /// <param name="commentText">The comment text to inspect</param>
    /// <returns><see langword="true"/> if the comment contains a supported marker; otherwise, <see langword="false"/></returns>
    private static bool ContainsAutoGeneratedMarker(string commentText)
    {
        return commentText.Contains("<auto-generated", StringComparison.OrdinalIgnoreCase)
               || commentText.Contains("<autogenerated", StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Determines whether the specified trivia represents a comment
    /// </summary>
    /// <param name="trivia">The trivia to inspect</param>
    /// <returns><see langword="true"/> if the trivia represents a regular or documentation comment; otherwise, <see langword="false"/></returns>
    ...
    private async Task<FileProcessResult> ProcessFileAsync(string filePath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (IsAutoGeneratedSource(syntaxTree, cancellationToken))
            {
                if (_verbose)
                {
                    _console.WriteLine($"Skipped (generated): {filePath}");
