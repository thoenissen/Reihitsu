...
internal sealed class FormatCommandHandler
{
    ...
    public async Task<int> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        var preScanResults = new Dictionary<string, FileInspectionResult>(pathComparer);
        var confirmationExitCode = await ConfirmLargeRunIfRequiredAsync(files, preScanResults, cancellationToken).ConfigureAwait(false);
        ...
        foreach (var filePath in files)
        {
            ...
            FileInspectionResult? preScanResult = preScanResults.TryGetValue(filePath, out var cachedResult)
                                                      ? cachedResult
                                                      : null;
            var processResult = await ProcessFileAsync(filePath, preScanResult, cancellationToken).ConfigureAwait(false);
    ...
    /// <param name="preScanResult">An optional successful inspection from large-run confirmation</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests</param>
    /// <returns>The processing counters for the file</returns>
    private async Task<FileProcessResult> ProcessFileAsync(string filePath,
                                                           FileInspectionResult? preScanResult,
                                                           CancellationToken cancellationToken)
    {
        try
        {
            var fileRead = preScanResult?.FileRead
                               ?? await _fileSystem.ReadFileAsync(filePath, cancellationToken).ConfigureAwait(false);
            var originalContent = fileRead.Content;
            var syntaxTree = preScanResult?.SyntaxTree
                                 ?? CSharpSyntaxTree.ParseText(originalContent, path: filePath, cancellationToken: cancellationToken);
            ...
            if (preScanResult?.IsAutoGenerated
                    ?? _formatter.IsAutoGeneratedSource(syntaxTree, cancellationToken))
            {
                if (_verbose)
                {
                    _console.WriteLine($"Skipped (generated): {filePath}");
                ...
                return FileProcessResult.SkippedGenerated;
            }
    ...
    /// <summary>
    /// Inspects a file using the data needed by both large-run planning and normal processing
    /// </summary>
    /// <param name="filePath">The file path to inspect</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests</param>
    /// <returns>A task containing the inspection result, or <see langword="null"/> when normal processing should retry the file</returns>
    private async Task<FileInspectionResult?> InspectFileAsync(string filePath, CancellationToken cancellationToken)
    {
        try
        {
            var fileRead = await _fileSystem.ReadFileAsync(filePath, cancellationToken).ConfigureAwait(false);
            var syntaxTree = CSharpSyntaxTree.ParseText(fileRead.Content, path: filePath, cancellationToken: cancellationToken);
            var isAutoGenerated = _formatter.IsAutoGeneratedSource(syntaxTree, cancellationToken);

            return new FileInspectionResult(fileRead, syntaxTree, isAutoGenerated);
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        {
            // Planning must not replace the normal per-file error and encoding handling. A file that cannot be
            // inspected remains a confirmation candidate and is processed through the usual path after confirmation.
            return null;
        }
    }
    ...
    /// <param name="preScanResults">Receives successful inspections for reuse during processing</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests</param>
    /// <returns>A task containing the exit code to return when the run must not continue; otherwise, <see langword="null"/></returns>
    private async Task<int?> ConfirmLargeRunIfRequiredAsync(List<string> files,
                                                            IDictionary<string, FileInspectionResult> preScanResults,
                                                            CancellationToken cancellationToken)
    {
        ...
        {
            var preScanResult = await InspectFileAsync(filePath, cancellationToken).ConfigureAwait(false);

            if (preScanResult is { } result)
            {
                preScanResults[filePath] = result;
