# Reihitsu.Cli

> 麗筆 (reihitsu, [ɾeːçitsɯ]) — beautiful brushwork

A command-line formatting tool for C# source files, powered by the Reihitsu formatter engine.

## Installation

```shell
dotnet tool install -g Reihitsu.Cli
```

## Usage

```shell
reihitsu-format [options] [<paths>...]
```

### Arguments

| Argument   | Description                                                              |
|------------|--------------------------------------------------------------------------|
| `<paths>`  | One or more files or directories to format. Defaults to the current directory if omitted. |

### Options

| Option      | Description                                                        |
|-------------|--------------------------------------------------------------------|
| `--check`   | Check if files are formatted correctly. Does not write changes. Returns exit code `1` if formatting is needed. |
| `--dry-run` | Show what would change without applying any modifications. Returns exit code `1` if changes would be made. |
| `--verbose` | Show detailed output for each file.                                |
| `--force`   | Skip the confirmation prompt that is shown when a run would format more than 25 files. Required for large runs when standard input is not interactive (e.g. CI). |
| `--utf8-bom` | Normalize processed files to UTF-8 with a byte order mark. Encoding-only changes are reported in `--check` and `--dry-run` modes. |
| `--lang-version <version>` | C# language version to parse and format for, such as `12`, `12.0`, `latest`, or `preview` (default: `latest`). `latest` selects C# 14.0, the newest version the formatter supports; newer values such as `preview` are lowered to 14.0 with a warning. Version-dependent rules follow it; for example, empty types keep their `{ }` body below C# 12. An invalid value fails with exit code `2`. `LangVersion` from project files is not read. |
| `--help`    | Show help message.                                                 |
| `--version` | Show version information.                                          |

### Exit Codes

| Code | Meaning                                        |
|------|------------------------------------------------|
| `0`  | Success — all files are formatted correctly.   |
| `1`  | Formatting needed (when using `--check` or `--dry-run`). |
| `2`  | Error occurred.                                |

## Examples

Format all C# files in the current directory:

```shell
reihitsu-format
```

Format a specific file:

```shell
reihitsu-format src/MyClass.cs
```

Check if files are properly formatted (useful for CI):

```shell
reihitsu-format --check src/
```

Preview changes without applying them:

```shell
reihitsu-format --dry-run src/
```

Show detailed output:

```shell
reihitsu-format --verbose src/
```

Format a large directory tree without the confirmation prompt:

```shell
reihitsu-format --force src/
```

Format files and normalize their encoding to UTF-8 with a byte order mark:

```shell
reihitsu-format --utf8-bom src/
```

Format a project that targets C# 11:

```shell
reihitsu-format --lang-version 11 src/
```
