---
name: gh-preflight
description: Run a read-only, local preflight review of a Reihitsu pull request before external review or re-review. Use for `/gh-preflight`, "preflight this PR", "review before review", or as the mandatory final quality gate inside `gh-implement` and `gh-apply-review` — invoked once their implementation, local self-review, and `origin/main` synchronization are done, and before their single full validation and CI-trigger commit. Apply the complete repository `gh-review` checklist, adversarial corpus, test expectations, counterpart tracing, and defect-class closure without posting to GitHub or changing code. Report every confirmed finding in one pass, because the parent gets one consolidated repair cycle and at most one retry attempt. Return a blocking gate when any confirmed finding remains.
---
