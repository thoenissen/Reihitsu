...
Use **one** Rubber Duck subagent per implementation run. When the user later clarifies the same contract, send the new evidence to that same subagent and ask for an amended contract — do not spawn a second one. Spawn a replacement only when the original agent is unavailable or the issue scope changes materially (a different defect, a different rule).

If subagents are unavailable in the current environment, perform the analysis yourself by following `gh-rubber-duck` before any edit, and record the resulting contract in this task. The gate still applies; only the isolation is lost.
