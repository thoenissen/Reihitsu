---
name: gh-implement
description: Orchestrator for implementing a Reihitsu GitHub issue end-to-end in the local Codex environment. Triggers when the initial prompt references a GitHub issue (e.g. "implement #123", "fix issue 45", or a github.com/.../issues/N URL). It reads the issue with the installed GitHub CLI, delegates the implementation to the matching repository command playbook (fix-formatter, fix-analyzer-rule, create-analyzer-rule, extend-formatter, create-rule-doc, add-resource-texts, draft-issue), runs the full validation suite, and opens a draft pull request using PULL_REQUEST_TEMPLATE.md.
---

# Implement GitHub Issue

You are running in the local Codex environment. The repository checkout, .NET SDK, and `gh` CLI are available. Your job is to take a single GitHub issue from "assigned" to "draft PR open", delegating the actual implementation to the repository's task-specific slash commands whenever one fits.
...
The local environment already provides the .NET SDK required by this `net10.0` repository and an authenticated `gh` CLI. Do not install an SDK or modify `PATH`. Confirm the GitHub CLI identity before making GitHub changes:
...
Use the installed `gh` CLI for every GitHub platform operation. Do not use GitHub MCP tools or raw `curl` calls to the GitHub API.
...
2. Make the change via the delegated command. Stage only the files that belong to this issue. Never `git add -A` blindly — the local worktree may contain unrelated user changes.
...
- **Never** modify `global.json` or the `TargetFramework` to work around a local toolchain failure.
- **Always** use the `gh` CLI for GitHub platform operations; GitHub MCP tools are not available in this environment.
...
- [ ] Branch pushed
- [ ] Draft PR opened via `gh pr create --draft` with `PULL_REQUEST_TEMPLATE.md` fully filled and `Closes #<N>`
