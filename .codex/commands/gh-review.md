...
Invoke the `gh-review` skill and follow it exactly. You run in a **Linux** cloud sandbox with no assumed `gh` CLI — all GitHub access goes through the GitHub MCP server (`mcp__github__*`):

1. Fetch PR metadata, diff, changed files, existing comments, and any linked issue via the GitHub MCP tools (`mcp__github__pull_request_read`, `mcp__github__issue_read`). The diff is not the review boundary — open counterpart files (analyzer ↔ formatter phase ↔ code fix) even when they are not in the diff.
