...
6. Run the read-only `gh-preflight` skill against the current pushed PR head. Fix every confirmed finding and the complete defect class in `[skip ci]` commits, then rerun preflight until it returns `PASS`. Do not post preflight findings to GitHub.
