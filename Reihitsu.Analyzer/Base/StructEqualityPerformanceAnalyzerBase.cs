...
public class StructEqualityPerformanceAnalyzerBase : DiagnosticAnalyzerBase
{
    ...
    /// <inheritdoc />
    internal StructEqualityPerformanceAnalyzerBase(string diagnosticId, DiagnosticCategory category, string titleResourceName, string messageFormatResourceName)
    ...
    protected static bool AreEqualityMembersImplemented(Compilation compilation, ITypeSymbol type)
    {
        ...
        foreach (var member in type.GetMembers())
        {
            if (member is IMethodSymbol method)
            {
                switch (method.Name)
                {
                    case "Equals" when method.Parameters.Length == 1
